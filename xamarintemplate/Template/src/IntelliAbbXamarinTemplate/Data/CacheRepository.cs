...
namespace IntelliAbbXamarinTemplate.Data
{
    public class CacheRepository : ICacheRepository
    {
        ...
        internal bool IsExpired(CacheEntry entry)
        {
            lock (pass)
            {
                if (!entry.ExpiresIn.HasValue)
                    return false;
#if DEBUG
                System.Diagnostics.Debug.WriteLine($"Cache {entry.Key} expires {entry.ExpiresIn.Value.Duration()} after creation.");
#endif
                ...
                {
#if DEBUG
                    System.Diagnostics.Debug.WriteLine($"Cache '{entry.Key}' expired {entry.ExpiresIn.Value.Duration()} after creation. Removing.");
#endif
                    Remove(entry);
                }
                else
